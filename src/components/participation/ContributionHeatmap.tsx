...
export function ContributionHeatmap({ compact = false, onSelectDistrict }: ContributionHeatmapProps) {
  ...
  if (compact) {
    return (
      <Card className="glass-intense border-white/10 p-4 sm:p-6">
          ...
          {districtParticipation.slice(0, 8).map((district) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "rounded-lg p-2 sm:p-3 text-center cursor-pointer hover:opacity-80 transition-opacity",
                  intensityColors[intensity],
                  "bg-opacity-20 border border-white/10"
                )}
                onClick={() => onSelectDistrict?.(district.district)}
              >
                <div className="text-[10px] sm:text-xs font-medium text-white truncate">{district.district}</div>
                <div className="text-xs sm:text-sm text-slate-300">{district.totalContributions}</div>
              </div>
            );
          })}
        </div>
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              <div className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-slate-500/20" />
              <div className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-amber-500/20" />
              <div className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-blue-500/20" />
              <div className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-green-500/20" />
              <div className="w-3 h-3 sm:w-4 sm:h-4 rounded bg-emerald-500/20" />
            </div>
  ...
  return (
    <Card className="glass-intense border-white/10 p-4 sm:p-5 md:p-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
        <h3 className="text-base sm:text-lg md:text-xl font-bold text-white flex items-center gap-2">
          <MapPin className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-emerald-400" />
          District Participation Heatmap
        </h3>
          ...
          {districtParticipation.map((district) => {
            ...
            return (
              <div
                key={district.district}
                className={cn(
                  "rounded-xl p-4 sm:p-5 cursor-pointer hover:scale-105 transition-all border w-full",
                  intensityColors[intensity],
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <MapPin className={cn("w-4 h-4 sm:w-5 sm:h-5", trendColor)} />
                  <TrendIcon className={cn("w-3.5 h-3.5 sm:w-4 sm:h-4", trendColor)} />
                </div>
                <div className="text-base sm:text-lg font-bold text-white mb-2 truncate">{district.district}</div>
                <div className="text-2xl sm:text-3xl font-bold text-white mb-1">{district.totalContributions}</div>
                <div className="text-sm text-slate-400 mb-3">contributions</div>
                <div className="flex items-center justify-between pt-3 border-t border-white/10">
                  <span className="text-sm text-slate-500 truncate">{district.members} members</span>
                  <Badge variant="outline" size="sm" className="text-xs flex-shrink-0 ml-2">
      ...
      {/* Legend */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mt-6 pt-6 border-t border-white/10">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <span className="text-xs sm:text-sm text-slate-400">Intensity:</span>
          <div className="flex gap-1 sm:gap-2 flex-wrap">
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded bg-slate-500/30" />
              <span className="text-xs sm:text-sm text-slate-500">&lt;20%</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded bg-amber-500/30" />
              <span className="text-xs sm:text-sm text-slate-500">20-40%</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded bg-blue-500/30" />
              <span className="text-xs sm:text-sm text-slate-500">40-60%</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded bg-green-500/30" />
              <span className="text-xs sm:text-sm text-slate-500">60-80%</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded bg-emerald-500/30" />
              <span className="text-xs sm:text-sm text-slate-500">80%+</span>
            </div>
          </div>
        </div>
