  ...
  return (
    <section className="py-24 bg-slate-950 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          {['All Entities', 'Protected Areas', 'Water Bodies', 'Species', 'Bloom Zones', 'Trails', 'Districts'].map((chip, idx) => (
            <button
              key={chip}
              className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all ${
                idx === 0
                  ? 'bg-gradient-to-r from-forest-600 to-forest-500 text-white shadow-lg shadow-forest-500/25'
                  : 'glass-light border border-white/10 text-slate-300 hover:text-white hover:border-forest-500/50'
              }`}
            >
              {chip}
            </button>
