      ...
      <main className="min-h-screen bg-slate-950"><section className="pt-48 pb-20"><div className="container mx-auto px-6 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="glass-intense border-white/10 p-8 text-center">
              ...
              <div className="bg-white/5 rounded-xl p-4 mb-6"><p className="text-sm text-slate-400 mb-1">Submission Reference ID</p><p className="text-xl font-mono font-bold text-indigo-400">{submissionId}</p></div>
            ...
            <div className="lg:col-span-2">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Card className="glass-intense border-white/10 p-6">
                  ...
                  <div className="space-y-4">
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Abstract / Summary *</label><textarea rows={4} required value={formData.abstract} onChange={e => handleChange('abstract', e.target.value)} placeholder="Brief summary of the research, key findings, and relevance" className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div><label className="block text-sm font-medium text-slate-300 mb-1">Time Range Start</label><input type="date" value={formData.timeRangeStart} onChange={e => handleChange('timeRangeStart', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
                      <div><label className="block text-sm font-medium text-slate-300 mb-1">Time Range End</label><input type="date" value={formData.timeRangeEnd} onChange={e => handleChange('timeRangeEnd', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
                      <div className="sm:col-span-2"><label className="block text-sm font-medium text-slate-300 mb-1">Geographic Coverage</label><input type="text" value={formData.geographicCoverage} onChange={e => handleChange('geographicCoverage', e.target.value)} placeholder="e.g., Kashmir Valley, specific districts" className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
                    </div>
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Methodology</label><textarea rows={2} value={formData.methodology} onChange={e => handleChange('methodology', e.target.value)} placeholder="Research methods, sampling design, analytical approach" className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
                  </div>
                </Card>
                <Card className="glass-intense border-white/10 p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Contributor Type *</label><select required value={formData.contributorType} onChange={e => handleChange('contributorType', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">{CONTRIBUTOR_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Contributor Name *</label><input type="text" required value={formData.contributorName} onChange={e => handleChange('contributorName', e.target.value)} placeholder="Your name or organization" className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" /></div>
                  </div>
                </Card>
                <Card className="glass-intense border-white/10 p-6">
