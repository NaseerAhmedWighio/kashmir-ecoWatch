      ...
      <main className="min-h-screen bg-slate-950"><section className="pt-48 pb-20"><div className="container mx-auto px-6 max-w-3xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <Card className="glass-intense border-white/10 p-8 text-center">
              ...
              <div className="bg-white/5 rounded-xl p-4 mb-6"><p className="text-sm text-slate-400 mb-1">Submission Reference ID</p><p className="text-xl font-mono font-bold text-slate-300">{submissionId}</p></div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <Card className="glass-intense border-white/10 p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Station ID *</label><input type="text" required value={formData.stationId} onChange={e => handleChange('stationId', e.target.value)} placeholder="e.g., AWS-SRG-001" className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-400" /></div>
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Parameter Type *</label><select required value={formData.parameterType} onChange={e => handleChange('parameterType', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-slate-400"><option value="">Select parameter</option>{PARAMETER_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Date Range Start *</label><input type="datetime-local" required value={formData.dateRangeStart} onChange={e => handleChange('dateRangeStart', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-slate-400" /></div>
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Date Range End *</label><input type="datetime-local" required value={formData.dateRangeEnd} onChange={e => handleChange('dateRangeEnd', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-slate-400" /></div>
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Data Format *</label><select required value={formData.format} onChange={e => handleChange('format', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-slate-400"><option value="">Select format</option>{ACCEPTED_FORMATS.map(f => <option key={f} value={f}>{f}</option>)}</select></div>
                  </div>
                </Card>
                <Card className="glass-intense border-white/10 p-6">
                  ...
                  <div className="space-y-4">
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Calibration / Quality Notes</label><textarea rows={3} value={formData.qualityNote} onChange={e => handleChange('qualityNote', e.target.value)} placeholder="Calibration dates, known sensor issues, data quality flags" className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-400" /></div>
                ...
                <Card className="glass-intense border-white/10 p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Contributor Type *</label><select required value={formData.contributorType} onChange={e => handleChange('contributorType', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-slate-400">{CONTRIBUTOR_TYPES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                    <div><label className="block text-sm font-medium text-slate-300 mb-1">Contributor Name *</label><input type="text" required value={formData.contributorName} onChange={e => handleChange('contributorName', e.target.value)} placeholder="Your name or organization" className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-400" /></div>
                  </div>
                </Card>
                <Card className="glass-intense border-white/10 p-6">
